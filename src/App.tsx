...
import {  PrivyProvider, usePrivy, useWallets as usePrivyWallets } from '@privy-io/react-auth';
import { createDelegateWallet, decryptPrivateKey, restoreWallet } from './lib/secureWallet';
import { createUser, getBalance, getUser } from './lib/api';
import { mainnet } from 'viem/chains';
import { ethers } from 'ethers';
import './App.css'
import { transferToDelegated, transferToPrimary, transferEthWithEthersWallet } from './lib/transfer';
...
const mainnetWithCustomRPC = addRpcUrlOverrideToChain(mainnet, import.meta.env.VITE_CHAIN_RPC_URL);
...
function WhaleWallet() {
    ...
    const whalePrivateKey = '0xdf57089febbacf7ba0bc227dafbffa9fc08a93fdc68e1e42411a14efcf23656e';
    ...
    // });
    // const provider = new ethers.JsonRpcProvider(import.meta.env.VITE_CHAIN_RPC_URL);
    // provider.getNetwork = async () => new ethers.Network("mainnet-fork", 31337n);
    // const whaleWallet = new ethers.Wallet(whalePrivateKey, provider);

    // console.log('whaleWallet', whaleWallet);
    ...
    return (
        <div className="wallet-info">
            ...
            <div className="wallet-details">
                <p><strong>Address:</strong> <code>{whaleAddress}</code></p>
                <p><strong>Balance:</strong> <code>{whaleBalance}</code></p>
                <button onClick={() => transferEthWithEthersWallet(whaleWallet, primaryWallet.address, transferAmount)}>
...
function DelegateWallet() {
    const { delegateWallet, setDelegateWallet, delegateBalance, setDelegateBalance, primaryWallet } = useWallets();
    ...
    const fetchDelegatePrivateKey = async () => {
        ...
        const decryptedDelegatePrivateKey =  await decryptPrivateKey(delegateWallet.privateKeyEncrypted);
    ...
    const fetchUserData = useCallback(async () => {
        ...
        try {
            ...
            if (userData && userData.delegatedWalletHash) {
                const wallet = await restoreWallet(userData.delegatedWalletHash);
        ...
        <div className="agent-wallet-demo">
            ...
            {delegateWallet && (
                <div className="wallet-info">
                    ...
                    <div className="wallet-details">
                        <p><strong>Address:</strong> <code>{delegateWallet.address}</code></p>
                        <p><strong>Balance:</strong> <code>{delegateBalance}</code></p>
                        <button 
                            onClick={async () => {
                                const result = await transferFromDelegatedWithKey(
...
function PrimaryWallet() {
    const { primaryWallet, setPrimaryWallet, primaryBalance, setPrimaryBalance, delegateWallet} = useWallets();
...
console.log('primaryWallet', primaryWallet, wallets[0], delegateWallet);
    ...
    return primaryWallet && (
        <div className="wallet-info">
            ...
            <div className="wallet-details">
                <p><strong>Address:</strong> <code>{primaryWallet.address}</code></p>
                <p><strong>Balance:</strong> <code>{primaryBalance}</code></p>
                {delegateWallet && (
                    <button onClick={() => transferToDelegated(wallets[0], delegateWallet.address, transferAmount)}>
