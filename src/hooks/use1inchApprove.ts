...
export function use1inchApprove(): UseApproveReturn {
    ...
    const check1inchAllowance = useCallback(
        async (
    ...
    const approve1inchIfNeeded = useCallback(
        async (
            addressToApprove: string,
            walletAddress: string,
            expectedAllowance: bigint,
            ...
            try {
                console.log(
                    'Insufficient allowance. Creating approval transaction...'
                );

                const approveTx =
                    await call1inchAPI<ApproveTransactionResponse>(
                ...
                const newAllowance = await check1inchAllowance(
                    addressToApprove,
                    walletAddress,
                    expectedAllowance
                );
